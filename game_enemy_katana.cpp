...
int EnemyKatana::m_textureId = -1;

EnemyKatana::EnemyKatana(DirectX::XMFLOAT2 position, DirectX::XMFLOAT2 size)
	: Enemy(position, size, ObjectTag::ENEMY_KATANA,Enemy_Type::KATANA)
{
	//当たり判定の分だけ事前に容量確保
...
void EnemyKatana::Finalize()
{
}

void EnemyKatana::Update(double elapsed_time)
{
	// ===== 死亡中の処理 =====
	if (m_isDead)
	{
...
void EnemyKatana::OnCollision(const GameObject* otherObject, const Collider* myCollider, const Collider* otherCollider, COLLISION_VEC hitVec)
{
...
void EnemyKatana::EnemyWalk()
{
	...
	const float stopRange = 50.0f;	// この座標まで近づいたら止まる
	...
	vel.y = GetVel().y; // 歩行中はY固定
	SetVel(vel);
}

void EnemyKatana::EnemyRun()
{
}

void EnemyKatana::EnemyJump()
{
...
void EnemyKatana::EnemyAtk()
{

}

void EnemyKatana::UpdateColliderPosition()
{
	if (m_bodyCollider)
	{
