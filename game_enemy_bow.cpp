...
int EnemyBow::m_textureId = -1;

EnemyBow::EnemyBow(DirectX::XMFLOAT2 position, DirectX::XMFLOAT2 size)
	: Enemy(position, size, ObjectTag::ENEMY_BOW, Enemy_Type::BOW)
{
	//当たり判定の分だけ事前に容量確保
...
void EnemyBow::Finalize()
{
}

void EnemyBow::Update(double elapsed_time)
{
	// ===== 死亡中の処理 =====
	if (m_isDead)
	{
...
void EnemyBow::OnCollision(const GameObject* otherObject, const Collider* myCollider, const Collider* otherCollider, COLLISION_VEC hitVec)
{
...
void EnemyBow::EnemyWalk()
{
	...
	const float stopRange = 300.0f;	// この座標まで近づいたら止まる
...
void EnemyBow::EnemyRun()
{
}

void EnemyBow::EnemyJump()
{
...
void EnemyBow::EnemyAtk()
{

}

void EnemyBow::UpdateColliderPosition()
{
	if (m_bodyCollider)
	{
